 ...
 */
public class Sniper_Machine extends Actor
{
    ...
    public void act() 
    {
            List<Robot> robotsInRange = getObjectsInRange(300, Robot.class);
            if(robotsInRange.size() != 0 && active == 1) {
                ...
                if(counter == 200) {
                   getWorld().addObject(new Bullet(getRotation(), robotInRange.getX(), robotInRange.getY(), robotInRange, 4), getX(), getY());
                   counter = 0;
                } else {
                   counter ++;
                }
